...
import griddler.importImage.ImportImage;
...
public class Main {
    ...
    private static void saveAs(Component parent) {
        ...
        if (!filter.accept(newFile)) {
            newFile = new File(newFile.getPath() + ".grd");
        }
        file = newFile;
        save(parent);
    }
    ...
    private static JMenuBar getMenuBar() {
        ...
        JMenuItem menuItemImportImage = new JMenuItem(new AbstractAction(TR.t("Import image")) {

            @Override
            public void actionPerformed(ActionEvent e) {
                new ImportImage(f, board.getData());
            }
        });
        menuItemSaveAs.setMnemonic(KeyEvent.VK_I);
        menuFile.add(menuItemImportImage);
